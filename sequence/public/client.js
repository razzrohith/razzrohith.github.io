...
document.addEventListener('DOMContentLoaded', () => {
  ...
  socket.on('roomCreated', ({ roomId: rid, isHost: hostFlag }) => {
    console.log('roomCreated event:', { roomId: rid, isHost: hostFlag });
    roomId = rid;
  ...
  socket.on('joinedRoom', ({ roomId: rid, isHost: hostFlag }) => {
    console.log('joinedRoom event:', { roomId: rid, isHost: hostFlag });
    roomId = rid;
    myPlayerId = socket.id;
    window.location.hash = roomId;
    el.roomCode.textContent = roomId;
  ...
  socket.on('roomUpdate', (room) => {
    console.log('roomUpdate received, gameStarted:', room.gameStarted);
    lastRoom = room;
    if (!room.gameStarted) {
      renderLobby(room);
    } else {
  ...
  el.createBtn.addEventListener('click', () => {
    ...
    console.log('Emitting createRoom', { userName });
  ...
  el.joinBtn.addEventListener('click', () => {
    ...
    if (!code) return;
    el.homeError.textContent = '';
    console.log('Emitting joinRoom', { roomId: code, userName: `Player${Math.floor(Math.random()*1000)}` });
  ...
  function renderLobby(room) {
    console.log('renderLobby: hostId=', room.hostId, 'myId=', myPlayerId);
    ...
    if (me) {
      ...
      if (me.team === null) {
        ...
        for (let i = 0; i < COLORS.length; i++) {
          ...
          if (teamCounts[i] >= 2) opt.disabled = true;
    ...
    // Start button: show only if current user is host AND at least 2 players
    const isHostNow = me ? me.isHost : false;
    const shouldShowStart = isHostNow && room.players.length >= 2;
    console.log('Start button shouldShow:', shouldShowStart, 'isHost:', isHostNow, 'players:', room.players.length);
  ...
  el.teamInput.addEventListener('change', () => {
    ...
    console.log('setTeam emitting', { roomId, team });
    socket.emit('setTeam', { roomId, team });
  });

  el.readyBtn.addEventListener('click', () => {
    console.log('toggleReady clicked, roomId:', roomId);
    if (!roomId) {
  ...
  // --- Global helpers ---
  window.isTwoEyedJack = (card) => card.rank === 'J' && (card.suit === '♥' || card.suit === '♦');
  window.isOneEyedJack = (card) => card.rank === 'J' && (card.suit === '♠' || card.suit === '♣');
