(async function () {
  ...
  function categories() {
    ...
    pageShell('Shop by category', 'Discovery map', 'Browse rich category rooms with active deals, store signals, and the hottest current offer in each lane.',
      `<section class="section-block"><div class="trust-strip">${stat('categories', data.categories.length, 'Balanced shopping lanes with current public inventory.')}${stat('active deals', totalDeals, 'Every card links into a filtered category route.')}${stat('top heat', Math.max(...data.deals.map((deal) => deal.heat)), 'Community momentum helps prioritize the feed.')}</div></section>
      <section class="section-block"><div class="category-card-grid">${data.categories.map((category) => {
        ...
        return `<a class="category-card tone-${category.tone} motion-item rich-category-card" href="./category.html?name=${encodeURIComponent(category.name)}"><span>${escapeHtml(category.icon)}</span><strong>${escapeHtml(category.name)}</strong><p>${escapeHtml(category.description)}</p><small>${deals.length} active deals / ${escapeHtml(stores)}</small>${top ? `<em>Top: ${escapeHtml(top.title)}</em>` : '<em>New deals arriving soon</em>'}</a>`;
      }).join('')}</div></section>`);
  }
  ...
  function stores() {
    pageShell('Store directory', 'Store watch', 'Follow stores, scan trust signals, compare coupon depth, and jump into dedicated store profiles.',
      `<section class="section-block"><div class="store-grid">${data.stores.map((store) => {
        ...
        return `<article class="store-card motion-item rich-store-card"><span>${escapeHtml(store.initials)}</span><h3><a href="./store.html?name=${encodeURIComponent(store.name)}">${escapeHtml(store.name)}</a></h3><p>${deals.length} active deals / ${coupons.length} coupons / ${compact(store.followers)} followers</p><div class="store-badges"><b>${store.rating} trust</b><b>${heat} heat</b></div><button type="button" class="${followed ? 'saved' : ''}" data-action="follow-store" data-store="${escapeHtml(store.name)}">${followed ? 'Following' : 'Follow store'}</button></article>`;
      }).join('')}</div></section>`);
  }
  ...
  function coupons() {
    ...
    pageShell('Coupon desk', 'Verified savings', 'Copy codes, filter by store, and scan expiration pressure without leaving the coupon workspace.',
      `<section class="section-block coupon-workspace">
        <div class="feed-toolbar"><div><p class="eyebrow">Coupon filters</p><h2>Find usable codes fast</h2></div><select id="couponStoreFilter" aria-label="Filter coupons by store"><option value="">All stores</option>${stores.map((store) => `<option value="${escapeHtml(store)}">${escapeHtml(store)}</option>`).join('')}</select></div>
  ...
  function renderCouponResults() {
    ...
    root.innerHTML = filtered.map((coupon) => `<article class="coupon-card motion-item"><div><strong>${escapeHtml(coupon.store)}</strong><code>${escapeHtml(coupon.code)}</code></div><p>${escapeHtml(coupon.description)}</p><footer><small>${coupon.verified ? 'Verified code' : 'Community tested'} / ${escapeHtml(coupon.expires)}</small><button class="copy-btn" type="button" data-action="copy" data-code="${escapeHtml(coupon.code)}">Copy code</button></footer></article>`).join('') || '<div class="empty-state"><strong>No coupons match</strong><p>Clear the store or status filter to see more codes.</p></div>';
  ...
  function renderAdminDealRow(deal) {
    ...
    const imageUrl = escapeHtml(deal.image_url || './assets/img/deals/monitor.svg');
    const submittedUrl = deal.deal_url ? escapeHtml(deal.deal_url) : '';
    return `
      <article class="admin-row admin-review-row motion-item" data-admin-status="${deal.status}" data-admin-moderation="${deal.moderation_status}">
        <a class="admin-thumb" href="${imageUrl}" target="_blank" rel="noopener noreferrer" aria-label="Open submitted image">
          <img src="${imageUrl}" alt="">
        </a>
        <div>
          <strong>${escapeHtml(deal.title)}</strong>
          <span>${escapeHtml(adminDealStore(deal))} / ${escapeHtml(adminDealCategory(deal))} / ${money(Number(deal.current_price || 0))} <del>${deal.original_price ? money(Number(deal.original_price)) : ''}</del> / ${deal.discount_percent || 0}% off</span>
          <small>Submitter ${escapeHtml(submitter)} / ${escapeHtml(deal.shipping_info || 'No shipping note')} / ${escapeHtml(deal.coupon_code || 'No coupon')}</small>
          <small>${escapeHtml(deal.description || 'No description')} ${deal.instructions ? `/ ${escapeHtml(deal.instructions)}` : ''}</small>
          ${submittedUrl ? `<a class="text-link" href="${submittedUrl}" target="_blank" rel="noopener noreferrer">Open submitted URL</a>` : ''}
        </div>
  ...
  function renderAdminReportRow(report) {
    return `
      <article class="admin-row motion-item" data-admin-status="${report.status}">
        <div>
          <strong>${escapeHtml(report.deals?.title || 'Reported deal')}</strong>
          <span>${escapeHtml(report.reason)} / reported by ${escapeHtml(adminReporter(report))}</span>
    ...
    return `
      <article class="admin-row motion-item" data-admin-status="${item.status}" data-admin-priority="${item.priority}">
        <div>
          ...
          <span>${escapeHtml(item.entity_type)} / ${escapeHtml(item.priority)} priority / ${adminStatus(item.status)}</span>
          <small>${escapeHtml(item.reason || 'No note')} / ${adminDate(item.created_at)}</small>
        </div>
        <div class="admin-badges">
          <b>${item.priority}</b>
    ...
    return `
      <article class="admin-row motion-item" data-admin-status="${thread.status}">
        <div>
          ...
          <span>${escapeHtml(thread.tag || 'General')} / ${adminStatus(thread.status)} / ${escapeHtml(profileLabel(thread))}</span>
    ...
    return `
      <article class="admin-row motion-item" data-admin-status="${post.status}">
        <div>
          ...
          <span>${adminStatus(post.status)} / ${escapeHtml(profileLabel(post))}</span>
  ...
  async function dashboard() {
    ...
    pageShell('Your dashboard', 'Member workspace', 'A private hub for your saved deals, posted deals, comments, alerts, and account profile.',
      `<section class="section-block"><div class="trust-strip">${stat('saved', savedDeals.length, 'Deals synced to your account.')}${stat('pending', postedCount('pending', 'pending'), 'Waiting for moderator review.')}${stat('approved', postedRows.filter((deal) => deal.moderation_status === 'approved').length, 'Approved or previously approved submissions.')}${stat('alerts', activeAlertRows.length, 'Active account alert rules.')}${stat('matches', alertMatches.length, 'Recent in-app alert matches.')}</div></section><section class="dashboard-grid"><article class="dashboard-card"><h3>Profile</h3><div class="profile-line"><strong>${escapeHtml(memberName)}</strong><span>${escapeHtml(Auth.user?.email || '')}</span></div><a class="deal-action" href="./post-deal.html">Post a deal</a><a class="ghost-button link-button" href="./alerts.html">Manage alerts</a></article><article class="dashboard-card"><h3>Saved deals</h3><div class="dashboard-list">${savedDeals.map((deal) => `<article><strong><a href="${deal.dealUrl}">${escapeHtml(deal.title)}</a></strong><span>${escapeHtml(deal.store)} / ${money(deal.currentPrice)}</span></article>`).join('') || '<p>No account saves yet.</p>'}</div></article><article class="dashboard-card wide-dashboard-card"><h3>Recent alert matches</h3><div class="dashboard-list alert-dashboard-list">${alertMatches.slice(0, 8).map(({ alert, deal, reasons }) => `<article><img src="${escapeHtml(deal.image)}" alt=""><div><strong><a href="${deal.dealUrl}">${escapeHtml(deal.title)}</a></strong><span>${escapeHtml(deal.store)} / ${money(deal.currentPrice)} / ${deal.discount}% off</span><small>${escapeHtml(alert.keyword || alertCategoryName(alert) || 'Alert')} matched by ${escapeHtml(reasons.join(', ') || 'current rule')}</small></div><a class="deal-action outbound-action" href="${outboundLink(deal, 'dashboard-alert')}" data-outbound-link>Get deal -></a></article>`).join('') || '<p>No alert matches yet. Create or broaden an alert to see matches here.</p>'}</div></article><article class="dashboard-card wide-dashboard-card"><h3>Submitted deals</h3><div class="dashboard-list submitted-list">${postedRows.map((deal) => `<article><img src="${escapeHtml(deal.image_url || './assets/img/deals/monitor.svg')}" alt=""><div><strong>${escapeHtml(deal.title)}</strong><span class="status-pill">${escapeHtml(deal.status)} / ${escapeHtml(deal.moderation_status)}</span><span>${money(Number(deal.current_price || 0))}${deal.original_price ? ` from ${money(Number(deal.original_price))}` : ''} / ${deal.discount_percent || 0}% off</span><small>Submitted ${adminDate(deal.created_at)} / updated ${adminDate(deal.updated_at)}</small></div></article>`).join('') || '<p>No submissions yet.</p>'}</div></article><article class="dashboard-card"><h3>Comments</h3><div class="dashboard-list">${commentRows.slice(0, 6).map((comment) => `<article><strong>${new Date(comment.created_at).toLocaleDateString()}</strong><p>${escapeHtml(comment.body)}</p></article>`).join('') || '<p>No comments yet.</p>'}</div></article><article class="dashboard-card"><h3>Deal alerts</h3><div class="dashboard-list">${alertRows.map((alert) => `<article><strong>${escapeHtml(alert.keyword || alertCategoryName(alert))}</strong><span>${escapeHtml(alertStoreName(alert))} / ${alert.max_price ? `under ${money(Number(alert.max_price))}` : 'any price'} / ${alert.min_discount_percent || 0}%+ off</span><small>${matchingDealsForAlert(alert).length} current matches / ${alert.is_active === false ? 'paused' : 'active'}</small></article>`).join('') || '<p>No alerts yet.</p>'}</div></article></section>`);
  }
